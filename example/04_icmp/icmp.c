...
static uint16_t ng_checksum(uint16_t *addr, int count) { //固定的icmp的checksum的和计算方式.
	...
	while (1) {
		...
		for (i = 0;i < num_recvd;i ++) {
			...
			if (iphdr->next_proto_id == IPPROTO_ICMP) {
				...
				struct rte_icmp_hdr *icmphdr = (struct rte_icmp_hdr *)(iphdr + 1);//指针移位，到达icmp的位置。

				struct in_addr addr;
				addr.s_addr = iphdr->src_addr;
				printf("icmp ---> src: %s ", inet_ntoa(addr));

				if (icmphdr->icmp_type == RTE_IP_ICMP_ECHO_REQUEST) {//icmp的请求报文

					addr.s_addr = iphdr->dst_addr;//如果是icmp的请求报文，源ip等于目标ip地址
