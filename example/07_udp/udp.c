...
#if ENABLE_MULTHREAD
/* ms:从in_ring中循环读取数据,解析数据，如果是icmp和arp数据直接生成报文放入到out_ring中,如果是udp的报文则将数据存放到recv buffer中 */
	...
	while (1) {
		...
		for (i = 0;i < num_recvd;i ++) {
			...
			if (iphdr->next_proto_id == IPPROTO_UDP) {
				...
				udp_process(mbufs[i]);//ms:解析出udp的数据并将数据存入到recv buffer中
...
static int udp_process(struct rte_mbuf *udpmbuf) {
	...
	/* ms:将数据解析到ol中，并将数据写入到recv buffer中 */
	rte_ring_mp_enqueue(host->rcvbuf, ol); //recv buffer

	pthread_mutex_lock(&host->mutex);
	pthread_cond_signal(&host->cond);//ms：这边的条件变量为什么要上锁。
...
// hook
/* 创建一个host，初始化fd、recv buffer 和send buffer,但是ip和端口没有初始化 */
...
static ssize_t nsendto(int sockfd, const void *buf, size_t len, __attribute__((unused))  int flags,
	...
	rte_ring_mp_enqueue(host->sndbuf, ol);//将数据放入到send buffer中
